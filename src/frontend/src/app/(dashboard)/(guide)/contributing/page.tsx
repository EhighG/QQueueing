  ...
  return (
    <div>
      ...
      <p>
        ...
        에서 기능을 설명하고, 왜 필요한지 그리고 어떻게 작동해야 하는지 설명하는
        이슈를 열어주세요.
      </p>
      <h2 id="-">코드 및 문서 변경 사항 기여</h2>
      <p>
      ...
      <ol>
        ...
        <li>
          ...
          <p>
            ...
            병합하기 전에 최종적으로 모든 커밋을 스쿼시하거나 리뷰어가 요청할 때
            우리가 대신 해줍니다.
          </p>
        </li>
        <li>
          ...
          <p>
            {" "}
            로컬 변경 사항을 포크한 저장소에 푸시하고 병합 요청을 제출하세요. 풀
            리퀘스트에서는 여러분이 한 변경 사항을 요약하는 제목을 선택하고,
            변경 사항에 대한 자세한 내용을 본문에 제공하세요. 또한 토론이 진행된
            이슈 번호를 언급하세요. 예: &quot;Closes #123&quot;.
          </p>
        </li>
      </ol>
      <p>
        그런 다음 기다리세요. 풀 리퀘스트에 대한 토론이 있을 것이며, 필요한 경우
        변경 사항을 함께 작업하여 여러분의 풀 리퀘스트를 QQueueing에 병합할
        것입니다. yaml 변경 로그 항목이 자동으로 생성됩니다. 외부 기여자가
        수동으로 편집할 필요가 없습니다. 리뷰어가 리베이스를 요청할 수 있지만,
        일반적으로 마지막에 스쿼시하는 것은 하지 않아야 합니다. 이것은 풀
        리퀘스트가 GitLab을 통해 통합될 때 수행할 수 있습니다.
      </p>
      <p>
        공개적으로 공유된 브랜치에 강제 푸시하지 않는 일반 지침을 준수하세요. 풀
        리퀘스트를 열면 브랜치가 공개적으로 공유된 것으로 간주됩니다. 강제 푸시
        대신 점진적인 커밋을 추가하세요. 이것이 리뷰어에게 일반적으로 더
        쉽습니다. 마스터에서 변경 사항을 가져와야 하는 경우 마스터를 브랜치로
        병합할 수 있습니다. 리뷰어가 긴 기간 동안 진행 중인 풀 리퀘스트에
        리베이스하라고 요청할 수 있으며, 그 경우 강제 푸시하는 것은 괜찮습니다.
        주의할 점은 리뷰 프로세스가 끝날 때 스쿼시하지 않아야 한다는 것입니다.
        이것은 풀 리퀘스트가{" "}
